...
from ..distribution.common import Softmax
from ..entropy_model.discrete import DiscreteUnconditionalEntropyModel, DiscreteConditionalEntropyModel
from compressai.models.vq.sim_vq import SimVQ
...
class NVTCSIMVQ(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        for s in reversed(range(self.n_stage)):
            ...
            for l in reversed(range(self.n_layer[s])):
                ...
                if self.use_vq[s][l]:
                    ...
                    # Quantize residual
                    quantized, indices, commit_loss = self.quantizer[s][l](r)

                    with torch.no_grad():
                        hist = torch.bincount(indices.flatten(), minlength=self.quantizer[s][l].codebook_size).float()
                        prob = hist / (hist.sum() + 1e-9)
                        entropy = -torch.sum(prob * torch.log2(prob + 1e-9))
                        bpp_est = entropy / (x_ori.shape[2] * x_ori.shape[3])

                    vq_dist.append(commit_loss)
                    rate_u.append(bpp_est)
                    rate_c.append(torch.tensor(0.0, device=r.device))
                    prior_vq_dist.append(torch.tensor(0.0, device=r.device))
                    prior_vq_rate.append(torch.tensor(0.0, device=r.device))
        ...
        # Statistics
        rate = sum(rate_u)
        distortion_loss = self.lmbda * ((x_ori - x_hat) ** 2).mean()
        vq_loss = self.lmbda * sum(vq_dist)
        rd_loss = distortion_loss + vq_loss
